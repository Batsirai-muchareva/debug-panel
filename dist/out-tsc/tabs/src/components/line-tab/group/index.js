...
const Group = ({ defaultActive, children, onChange }) => {
    ...
    const items = react_1.default.Children.toArray(children).filter(react_1.default.isValidElement);
    const activeIndex = items.findIndex((child) => child.props.id === active);
    function activate(id) {
        setActive(id);
        onChange === null || onChange === void 0 ? void 0 : onChange(id);
    }
    return ((0, jsx_runtime_1.jsxs)("div", { className: group_module_scss_1.default.group, children: [(0, jsx_runtime_1.jsx)(indicator_1.Indicator, { count: items.length, index: activeIndex }), items.map((child) => {
                const item = child;
